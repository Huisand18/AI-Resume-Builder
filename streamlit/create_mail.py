...
# Fungsi untuk menerjemahkan prompt pengguna ke bahasa Inggris
...
if submit_button:
    if user_input.strip() == "done":
        ...
        ai_response = interact_with_ai(translated_input, context_bot2)
        download_button = True 
        
        st.text_area("Result:", value=ai_response, height=200)
        ...
        if "The bot can only assist in email creation. Please enter the email related prompt." not in ai_response:
            if download_button:  # Menampilkan tombol unduh jika respons tersedia
                file_name = save_to_word(ai_response, bot_option)  # Simpan respon terjemahan ke dalam dokumen Word
